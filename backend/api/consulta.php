...
$patronesComando = [
    '/^(ok|entendido)$/i',  // 🔧 QUITADOS: continuar, listo, siguiente, si, sí, no
...
function esPreguntaValidaParaEstadisticas($question) {
    ...
    // 1. Filtro por longitud mínima (menos de 10 caracteres probablemente no es pregunta)
    if ($longitudPregunta < 10) {
        return false;
    }
    
    // 2. Filtro por longitud máxima (más de 500 caracteres probablemente es texto pegado)
    if ($longitudPregunta > 500) {
        return false;
    }
    
    // 3. Detectar si es un comando
    if (detectarComandoMejorado($question) !== null) {
        return false;
    }
    ...
    // 7. Verificar que contenga al menos una palabra interrogativa o contexto de pregunta
    $esInterrogativa = 
        preg_match('/\?$/', $questionTrimmed) || // Termina en ?
        preg_match('/^(qu[eé]|c[oó]mo|cu[aá]ndo|d[oó]nde|por qu[eé]|cu[aá]l|qui[eé]n|para qu[eé])/i', $questionLower) || // Palabras interrogativas
        preg_match('/(explica|define|dime|indica|describe|menciona|enumera|lista)/i', $questionLower) || // Verbos de solicitud
        preg_match('/(puedes|podrías|me ayudas|necesito|quisiera|quiero saber)/i', $questionLower); // Solicitudes
    
    // Si no tiene ningún indicador de pregunta y es muy corta (menos de 20 chars), rechazar
    if (!$esInterrogativa && $longitudPregunta < 20) {
        return false;
    }
    
    // 8. Filtrar contenido repetitivo o sin sentido
    $palabrasUnicas = array_unique($palabras);
    $ratioRepeticion = count($palabrasUnicas) / count($palabras);
    
    // Si más del 50% de palabras están repetidas, probablemente no es válido
    if ($ratioRepeticion < 0.5) {
        return false;
    }
    
    // Si pasa todos los filtros, es una pregunta válida
    return true;
}
